module enemy_grid1(
   ...
   output reg bullet_hit,
	...
	output [49:0] dead_out,
	output [9:0] grid_x_out,
	output [9:0] grid_y_out
);
...
assign grid_x_out = grid_x;
assign grid_y_out = grid_y;
...
begin
    ...
    for (k = 0; k < ENEMIES; k = k + 1) begin
        if (!dead[k]) begin
            ...
            if (ay + ENEMY_H > lowest_enemy_y)
                lowest_enemy_y = ay + ENEMY_H;
        end
    end

    if (lowest_enemy_y >= 400)
        enemy_reached_bottom = 1;
end
        ...
        case (S)
            ...
            SHIFT_D: begin
               if(right == 1)
						...
						right <= 1'b1;
                grid_y <= grid_y + SPACING_Y;
					 if(right == 1 && speed_inc < 20)
